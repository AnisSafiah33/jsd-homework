...
//     //console.log(item);
//     document.body.innerHTML += `<p>${item.term}</p>`
//   }
//   )
  ...
  function inputOnChange(event) {
    ...
    glossary
        .filter((item) => item.term.toUpperCase() === term.toUpperCase())
        .forEach((item) => {
...
let groupedClass = groupBy(glossary, 'class');
...
function getDropDownList() {
    ...
    for (let j = 0; j < Object.keys(groupedClass).length; j++) {
        let option = document.createElement('option')
        option.innerHTML = `${Object.keys(groupedClass)[j]}`
...
function selectOnChange(event) {
    ...
    glossary
        .filter((item) => parseInt(item.class) === parseInt(selectedClass))
        .forEach((item) => {
            divResult.innerHTML += `${item.term} <br>`
        })
}
...
function getCheckbox() {
    ...
    let combineAllTagsArray = glossary
        .map(a => a.tags)
        .reduce((a, currentValue) => {
            return a.concat(currentValue)
        }, []);

    combineAllTagsArray
        .filter((item, pos) => {
            return combineAllTagsArray.indexOf(item) == pos;
        })
        .forEach((item) => {
            div.innerHTML += `<input type="checkbox" id="box${item}"> ${item} `
        })
}
...
function checkboxOnChange(event) {
    ...
    for (let i = 0; i < selectedCheckbox.length; i++) {
        selectedCheckboxArray.push(selectedCheckbox[i].id.substring(3))
    }
    ...
    glossary
        .forEach((item) => {
            ...
            item.tags.forEach((tag) => {
                if (selectedCheckboxArray.includes(tag)) {
                    boolTag = true;
                }
            })
...
for (let k = 0; k < checkbox.length; k++) {
    checkbox[k].addEventListener('change', checkboxOnChange)
}